...
pub fn create_table(sq: Square, rook: bool) -> String {
    ...
    let magic = if rook {
        MAGIC_NUMBERS_ROOK[sq as usize]
    } else {
        MAGIC_NUMBERS_BISHOP[sq as usize]
    };
...
pub fn save() {
    let mut file_str = format!(
		...
		use crate::prelude::*;
		const MOVEMENTS_ROOK: [Bitboard; 64] = {};
		const MOVEMENTS_BISHOP: [Bitboard; 64] = {};
		const ATTACKS_ROOK: [&'static [Bitboard]; 64] = {};
		const ATTACKS_BISHOP: [&'static [Bitboard]; 64] = {};
	",
